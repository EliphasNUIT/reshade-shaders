...
float3 ApplyLUT(float4 position : SV_Position, float2 texcoord : TexCoord) : SV_Target {
	...
	if (DebugLumaOutputHQ) {
		...
		return tex2D(LumaInputSampler, texcoord.xy).x;
	}
...
/*float3 RenderLightMap(float4 position : SV_Position, float2 texcoord: TexCoord) : SV_Target {
	return tex2D(ReShade::LightBuffer, texcoord.xy).x;
}*/
...
/*technique ShowLightMap {
	pass LightMap {
		VertexShader = PostProcessVS;
		PixelShader = RenderLightMap;
	}
}*/
